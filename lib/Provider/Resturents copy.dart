...
class Resturents with ChangeNotifier {
  ...
  Future<void> addResturent(Resturent resturent, download) async {
    try {
  ...
  Future<void> updateResturent(Resturent resturent, [donwload = ""]) async {
    try {
      ...
      if (prodIndex >= 0) {
        ...
        await http.put(url,
            ...
            body: json.encode({
              'Name': resturent.name,
              'description': resturent.description,
              'imageUrl': donwload != "" ? donwload : resturent.imageUrl,
              'Loocation': resturent.location,
            }));
